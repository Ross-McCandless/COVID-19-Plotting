...
G7Countries_df = sum_df.nlargest(7, 'Cases%')
print(G7Countries_df)

# plotting
plt.plot(G7Countries_df['countriesAndTerritories'],
         G7Countries_df['Cases%'], marker='o', color='red')
for x, y in zip(G7Countries_df['countriesAndTerritories'], G7Countries_df['Cases%']):
    plt.annotate(str(round(y, 2)) + "%", xy=(x, y))
plt.title("G7 Countries COVID-19 Infection rates")
plt.xlabel("Country")
