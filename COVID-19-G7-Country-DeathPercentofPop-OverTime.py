...
sum_df['cumcases'] = sum_df.groupby('countriesAndTerritories')[
    'deaths'].transform(pd.Series.cumsum)
sum_df['PercentDeceased'] = sum_df['cumcases'] / sum_df['popData2018'] * 100
...
plt.plot(canada_df['dateRep'],
         canada_df['PercentDeceased'], marker='o', color='red', label='Canada ({})'.format(canada_df['cumcases'].max()))
plt.plot(us_df['dateRep'],
         us_df['PercentDeceased'], marker='o', color='blue', label='USA ({})'.format(us_df['cumcases'].max()))
plt.plot(germany_df['dateRep'],
         germany_df['PercentDeceased'], marker='o', color='gold', label='Germany ({})'.format(germany_df['cumcases'].max()))
plt.plot(italy_df['dateRep'],
         italy_df['PercentDeceased'], marker='o', color='green', label='Italy ({})'.format(italy_df['cumcases'].max()))
plt.plot(japan_df['dateRep'],
         japan_df['PercentDeceased'], marker='o', color='darkorange', label='Japan ({})'.format(japan_df['cumcases'].max()))
plt.plot(united_kingdom_df['dateRep'],
         united_kingdom_df['PercentDeceased'], marker='o', color='black', label='UK ({})'.format(united_kingdom_df['cumcases'].max()))
plt.plot(france_df['dateRep'],
         france_df['PercentDeceased'], marker='o', color='pink', label='France ({})'.format(france_df['cumcases'].max()))
plt.legend(loc='upper center', title='Country (# of deaths)')
plt.title("G7 Countries COVID-19 Deaths in Population (%) over time")
plt.xlabel("Date (from: 2020-02-15 to: {})".format(datetime.date.today()))
plt.ylabel("Deaths in Population (%)")
